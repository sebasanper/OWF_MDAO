# -*- coding: utf-8 -*-
"""
Created on Wed Nov 11 18:22:45 2015

@author: Αλβέρτος
"""

from math import pi, sqrt, sinh, tanh, gamma, exp
...
class SiteConditionsAnalysts:
    ...
    def set_conditions(self):
        ...
        self.site_conditions.Hmax_50_year = min(self.site_conditions.Hmax_50_year, self.wave_limit(self.site_conditions.kmax_50_year))
        ...
        self.site_conditions.Uw_50_year = (pi * self.site_conditions.Hs_50_year /
                                              (self.site_conditions.Tpeak_50_year * sinh(self.get_wave_number(self.site_conditions.Tpeak_50_year) * self.site_conditions.water_depth))) 
        ...
        self.site_conditions.Hred_50_year = min(self.site_conditions.Hred_50_year, self.wave_limit(self.site_conditions.kred_50_year))
        ...
        self.site_conditions.Hmax_1_year = min(self.site_conditions.Hmax_1_year, self.wave_limit(self.site_conditions.kmax_1_year))
    ...
    def set_Vaverage(self):
        self.site_conditions.Vaverage = self.site_conditions.scale_factor * gamma(1.0 + 1.0 / self.site_conditions.shape_factor)
    ...
    def set_wind_pdfs(self):
        ...
        for wd in range(len(_wind_directions)):
            ...
            if wd == 0:
                _sector_factor = 0.5 * (_wind_directions[wd + 1] - _wind_directions[(len(_wind_directions) - 1)]) / (2 * pi) 
            ...
            if _sector_factor < 0.0:
                _sector_factor = _sector_factor + 0.5 # Factor 0.5 is the result of + pi / (2 * pi)
            ...
            for ws in range(len(_wind_speeds)):
                ...
                _probability_density = (k / c) * (_wind_speeds[ws] / c)**(k - 1.0) * exp(-1.0 * (_wind_speeds[ws] / c)**k)
    ...
    def get_wind_direction(self, index):
        ...
        _wind_direction = (float(index) / float(self.number_of_sectors)) * 2.0 * pi + self.support_team.wind_farm_orientation * pi / 180.0
        if _wind_direction >= 2.0 * pi:
            _wind_direction = _wind_direction - 2.0 * pi
    ...
    def dispersion(self, d, *args):
        ...
        return omega**2 - self.g * k * tanh(k * self.site_conditions.water_depth)
